...
public class LeavesAtSameLevel {
	...
	{
		if(checkUtil(tree,0)==-1)
			return false;
		else
			return true;
	}
	
	static int checkUtil(Tnode root,int n)
	{
		int n1=0, n2=0;
		if(n==-1)
			return -1;                // for not at same level
		if(root.left==null&&root.right==null)
			return n+1;               // for leaf node(basically return 1)
		if(root.left!=null)
			n1=checkUtil(root.left,n); // for calculating height of left subtree
		if(root.right!=null)
			n2=checkUtil(root.right,n);       // for calculating height of right subtree
